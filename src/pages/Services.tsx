    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {services.map((service) => {
          ...
          return (
            <div key={service.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm flex flex-col">
              <div className="bg-slate-50 dark:bg-slate-800/50 p-3 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
                <h4 className="font-bold text-slate-800 dark:text-white text-sm">{service.name}</h4>
                <button
                  onClick={() => {
                    setConfirmModal({
                      isOpen: true,
                  ...
                  className="text-red-500 p-1 hover:bg-red-50 dark:hover:bg-red-900/30 rounded"
                >
              ...
              <div className="flex-1 overflow-y-auto max-h-64">
                {serviceBanks.length === 0 ? (
                  <p className="p-4 text-sm text-slate-500 dark:text-slate-400 italic">Aucun point de collecte.</p>
                ) : (
                  <ul className="divide-y divide-slate-100 dark:divide-slate-700">
                    {serviceBanks.map((bank) => (
                      <li key={bank.id} className="flex items-center justify-between p-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                        <Link
                          to={`/banks/${bank.id}`}
                          className="flex-1 flex items-center justify-between pr-4"
                        >
                          <span className="text-slate-700 dark:text-slate-200 text-sm font-medium">{bank.name}</span>
                          <FontAwesomeIcon icon={faChevronRight} className="text-slate-300 dark:text-slate-500 text-xs" />
                        </Link>
                        <button
                          onClick={() => {
                            setConfirmModal({
                          ...
                          className="text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 p-2 -mr-2 rounded transition-colors"
                        >
                          <FontAwesomeIcon icon={faTrash} className="text-sm" />
                        </button>
                      </li>
